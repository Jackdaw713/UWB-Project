...
#define APP_NAME "DSTWR RESP MULTI-NODE v2.1 (32-BIT)"

/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   DİKKAT: BU KARTI IKINCI MODÜLE YÜKLERKEN BURAYI 2 YAP !!!
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
#define MY_RESPONDER_ID 3   
...
/* Responder 3 Icin Hassas Anten Kalibrasyonu */
#define TX_ANT_DLY 16353
#define RX_ANT_DLY 16353
...
/* YENI: Rapor mesaji 4 byte data alanina cikarildi (Toplamda 2 byte eklendi) */
static uint8_t tx_report_msg[] = {0x41, 0x88, 0, 0xCA, 0xDE, 0xAA, 0xAA, MY_RESPONDER_ID, 0x00, 0x24, 0, 0, 0, 0}; 
...
#define FINAL_RX_TO_REPORT_TX_DLY_UUS 18000 
...
int app_main(void)
{
    ...
    LOG_INF("Responder [ID: %d] Hazir ve Dinliyor! (32-Bit Modu)", MY_RESPONDER_ID);
                    ...
                    if (status_reg & SYS_STATUS_RXFCG_BIT_MASK) {
                        ...
                        if (dwt_readstsquality(&stsqual)) {
                            ...
                            if (memcmp(rx_buffer, rx_final_msg, ALL_MSG_COMMON_LEN) == 0) {
                                ...
                                /* YENI ADIM: VERIYI 32-BIT OLARAK PAKETLE VE FIRLAT */
                                uint32_t dist_mm = (uint32_t)(distance * 1000.0); 
                                tx_report_msg[REPORT_MSG_DIST_IDX]     = dist_mm & 0xFF;         
                                tx_report_msg[REPORT_MSG_DIST_IDX + 1] = (dist_mm >> 8) & 0xFF; 
                                tx_report_msg[REPORT_MSG_DIST_IDX + 2] = (dist_mm >> 16) & 0xFF; 
                                tx_report_msg[REPORT_MSG_DIST_IDX + 3] = (dist_mm >> 24) & 0xFF; 
                                ...
                                int rep_ret = dwt_starttx(DWT_START_TX_DELAYED); 
