...
#include <math.h> /* pow() ve sqrt() fonksiyonlari icin eklendi */

#define LOG_LEVEL 3
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(ds_twr_sts_sdc_init);

#define APP_NAME "DSTWR INIT 2D TRILATERATION v1.0"
...
/* Sabit Kalibrasyon (Gerekirse ileride degistirirsin) */
...
void calculate_2d_position(void) {
    double d1 = distances[0]; // Node 1 (0,0) mesafesi
    double d2 = distances[1]; // Node 2 (1.2, 0) mesafesi
    double d3 = distances[2]; // Node 3 (0, 0.8) mesafesi

    /* Çember Kesişim Matematiği (Optimizasyonlu Formül) */
    /* X Koordinatı Hesaplama */
    double pos_x = (pow(d1, 2) - pow(d2, 2) + pow(ANCHOR_2_X, 2)) / (2.0 * ANCHOR_2_X);
    
    /* Y Koordinatı Hesaplama */
    double pos_y = (pow(d1, 2) - pow(d3, 2) + pow(ANCHOR_3_X, 2) + pow(ANCHOR_3_Y, 2) - (2.0 * pos_x * ANCHOR_3_X)) / (2.0 * ANCHOR_3_Y);

    LOG_INF("+++ KONUM: [ X: %.2f m | Y: %.2f m ] +++", pos_x, pos_y);
}
...
int app_main(void)
{
    ...
    LOG_INF("Basliyor... 2D Trilaterasyon Modu Aktif! (Filtresiz Ham Veri)");
                ...
                if (ret == DWT_SUCCESS) {
                    ...
                    if (status_reg & SYS_STATUS_RXFCG_BIT_MASK) {
                        ...
                        if (memcmp(rx_buffer, rx_report_msg, ALL_MSG_COMMON_LEN) == 0) {
                            ...
                            /* Ufak bi offset (Kalibrasyon istersen buraya ekle) */
                            final_distance -= 0.08; 
                            ...
                            /* Ham mesafeyi de gormek istersen acik kalsin */
                            // LOG_INF("[ID: %d] Ham Mesafe: %.2f", current_target_id, final_distance);

                            /* Eger 3 Nodedan da en az 1 kere veri geldiyse konumu hesapla */
